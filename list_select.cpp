...
ListSelect::ListSelect(stringvec& items_in, int pageSize_in)
    : items(items_in), pageSize(pageSize_in) {
  items = items_in;
...
void ListSelect::Draw(SDL_Surface* screen) {
  ...
  for (it = first_onscreen; it != last_onscreen; it++) {
    ...
    apply_surface(LEFT_MARGIN, item * (FONT_SIZE + PIXEL_SPACING) + TOP_MARGIN,
                  message, screen);
...
std::string ListSelect::GetCurrent() { return *selected_item; }

int main2() {
  Background background =
      Background("/usr/share/bunnymenu/background.jpg", 320, 240, 32);
  TTF_Init();
  stringvec list;
  for (int i = 0; i < 8; i++) {
    list.push_back("foo");
